...
namespace DcsBios {
    ...
    private:
        ...
        void decodeFromStream(pb_istream_t *stream) {
            ...
            slave_message.slave.id.funcs.decode = &decode_to_string;
            slave_message.slave.id.arg = &slave_details.id;
            slave_message.slave.mac.funcs.decode = &decode_to_string;
            slave_message.slave.mac.arg = &slave_details.mac;

            isValid = pb_decode(stream, SlaveMessage_fields, &slave_message);

            if (isValid) {
                seq = slave_message.seq;
                slave_details.rssi = slave_message.slave.rssi;
                slave_details.free_heap = slave_message.slave.free_heap;
                slave_details.loop_duration = slave_message.slave.loop_duration;
                slave_details.cpu_freq = slave_message.slave.cpu_freq;
                slave_details.flash_size = slave_message.slave.flash_size;
            }
        }
    public:
        class Slave {
        ...
        Message(AsyncUDPPacket& packet) : isValid(false) {            
            ...
            bool encodeToVector(std::vector<unsigned char> &vec) {
                ...
                slave_message.slave.id.funcs.encode = &encode_to_string;
                slave_message.slave.id.arg = &slave_details.id;
                slave_message.slave.mac.funcs.encode = &encode_to_string;
                slave_message.slave.mac.arg = &slave_details.mac;

                slave_message.seq = seq;
                slave_message.slave.rssi = slave_details.rssi;
                slave_message.slave.free_heap = slave_details.free_heap;
                slave_message.slave.loop_duration = slave_details.loop_duration;
                slave_message.slave.cpu_freq = slave_details.cpu_freq;
                slave_message.slave.flash_size = slave_details.flash_size;

                return pb_encode(&stream, SlaveMessage_fields, &slave_message);
            }
    };
}
